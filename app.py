...
totem_general = {
    ...
    "藍猴": {
        ...
        "design": "不對稱耳環 (Asymmetrical)、可拆卸組合的Charm墜飾、童趣圖案 (如小動物)",
        "metal": "K白金",
        "vibe": "遊戲、幻象、幽默"
    },
    "藍鷹": {
...
# ────────────── 2. Tru-Mi Product Mapping (品牌產品對應) ──────────────
# 包含：Tru-Mi 系列名、推薦單品、行銷文案、購買連結
totem_trumi = {
    "紅龍": {
        "series": "Memory 系列 (記憶)",
...
with col1:
    ...
    if general_rec:
        with st.container(border=True):
            ...
            st.info("💡 **能量原理**：選擇含有這些元素的飾品，能協助妳放大原生天賦，與靈魂頻率共振。")

# 右欄：Tru-Mi 品牌對應 (產品推薦)
with col2:
    st.markdown("#### 💍 Tru-Mi 系列推薦 (Collection)")
    st.caption("為妳的故事挑選專屬珠寶")
...
st.caption(f"由妳的銀河調性 {tone_number} 決定")
...
with col_tone:
    with st.container(border=True):
        ...
        st.markdown(f"**🎯 推薦結構：{tone_rec['structure']}**")
        st.write(tone_rec['style'])

with col_expert:
    st.info("✨ **專家推薦：今日能量選品**")
    summary_text = f"""
    想像一下，戴上一款 **{general_rec.get('metal', 'K金')}** 的 **{tone_rec.get('structure', '飾品')}**。
    
    主石選用 **{general_rec.get('gem', '').split('、')[0]}**，並在 **Tru-Mi 的 {trumi_rec.get('series', '').split(' ')[0]}** 中尋找靈感。
    這不僅是一件飾品，更是啟動妳「{general_rec.get('vibe', '')}」能量的專屬按鈕。
    """
